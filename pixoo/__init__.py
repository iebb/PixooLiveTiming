...
class Pixoo:
    ...
    def draw_character(self, character, xy=(0, 0), rgb=Palette.WHITE):
        matrix, w = retrieve_glyph(character)
        for dx, dy in matrix:
            self.draw_pixel((xy[0] + dx, xy[1] + dy), rgb)
        return w

    def draw_character_at_location_rgb(self, character, x=0, y=0, r=255, g=255, b=255):
        return self.draw_character(character, (x, y), (r, g, b))
    ...
    def draw_text(self, text, xy=(0, 0), rgb=Palette.WHITE):
        ...
        for index, character in enumerate(text):
            sum_l += self.draw_character(character, (sum_l + xy[0], xy[1]), rgb)

    def length_text(self, text):
        return sum(retrieve_glyph(character)[1] for character in text)
